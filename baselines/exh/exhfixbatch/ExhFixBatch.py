...
from ..utils import PromptGenerator, MctsPromptGenerator, UniversalMctsPromptGenerator, TestPromptGenerator
import time
...
class ExhFixBatch(SingleBehaviorRedTeamingMethod):
    def __init__(self, 
                 judge_model,
                 target_model,
                 batch_size=128,
        ...
        # Initialization
        self.prompt_generator = TestPromptGenerator(self.judge_modle_config, self.target_model_config, self.response_length, self.prompt_length_threshold, self.threshold, self.chat_template, self.use_chat_template)
    ...
    def generate_test_cases_single_behavior(self, behavior_dict, verbose, **kwargs):
        ...
        test_case, response = self.prompt_generator.generate_adv_prompt(test_case, batch_size, self.top_p, self.top_k, behavior_dict)

        running_time, running_seconds = running_time_record(start_time)

        logs.append(dict(test_case=test_case, number_of_tried_prompts=self.prompt_generator.number_of_tried_prompts, successful_attack=self.prompt_generator.successful_attack, running_time=running_time, running_seconds=running_seconds, response=response))
